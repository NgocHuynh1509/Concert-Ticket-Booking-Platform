...
public class DataSeederConfig {
    ...
    public CommandLineRunner initDatabase(EntityManager entityManager, PlatformTransactionManager transactionManager,
        ...
        return args -> new TransactionTemplate(transactionManager).executeWithoutResult(status -> {
            ...
            if (count(entityManager, "User") == 0) {
                seedUsers(entityManager, now, passwordEncoder);
    ...
    private void seedUsers(EntityManager entityManager, LocalDateTime now, PasswordEncoder passwordEncoder) {
        String defaultPassword = passwordEncoder.encode("123456");
        List<User> users = Arrays.asList(
                touch(User.builder().username("customer1").email("customer1@gmail.com").passwordHash(defaultPassword)
                        .role(UserRole.CUSTOMER).build(), now),
                touch(User.builder().username("customer2").email("customer2@gmail.com").passwordHash(defaultPassword)
                        .role(UserRole.CUSTOMER).build(), now),
                touch(User.builder().username("operator_nv").email("operator@geekup.vn").passwordHash(defaultPassword)
                        .role(UserRole.OPERATOR).build(), now),
                touch(User.builder().username("admin_root").email("admin@geekup.vn").passwordHash(defaultPassword)
                        .role(UserRole.ADMIN).build(), now));
    ...
    private void seedVouchers(EntityManager entityManager, LocalDateTime now) {
        List<Voucher> vouchers = Arrays.asList(
                touch(createVoucher("EARLYBIRD20", DiscountType.PERCENTAGE, "20", 500, 1, now.minusDays(1),
                        now.plusDays(30)), now),
                touch(createVoucher("FLASH500K", DiscountType.FIXED_AMOUNT, "500000", 100, 1, now, now.plusDays(3)),
                        now),
                touch(createVoucher("WELCOME10", DiscountType.PERCENTAGE, "10", 1000, 2, now.minusDays(5),
                        now.plusMonths(6)), now),
                touch(createVoucher("VIPONLY1M", DiscountType.FIXED_AMOUNT, "1000000", 50, 1, now, now.plusDays(7)),
                        now),
                touch(createVoucher("GEEKUP26", DiscountType.PERCENTAGE, "26", 200, 1, now.minusDays(2),
                        now.plusDays(14)), now),
                touch(createVoucher("NIGHTOWL", DiscountType.FIXED_AMOUNT, "200000", 300, 3, now, now.plusDays(10)),
                        now),
                touch(createVoucher("STUDENT15", DiscountType.PERCENTAGE, "15", 5000, 2, now.minusMonths(1),
                        now.plusMonths(5)), now),
                touch(createVoucher("SUMMERVIBE", DiscountType.FIXED_AMOUNT, "150000", 800, 1, now, now.plusMonths(1)),
                        now),
                touch(createVoucher("WEEKEND", DiscountType.PERCENTAGE, "5", 2000, 5, now, now.plusDays(2)), now),
                touch(createVoucher("BLACKFRIDAY", DiscountType.PERCENTAGE, "50", 10, 1, now.plusDays(10),
                        now.plusDays(11)), now));
    ...
    private void seedConcerts(EntityManager entityManager, LocalDateTime now) {
        ...
        for (int i = 0; i < concertNames.size(); i++) {
            Concert concert = touch(Concert.builder()
                    ...
                    .description("Siêu đại nhạc hội " + concertNames.get(i)
                            + " với những màn trình diễn đỉnh cao và dàn âm thanh ánh sáng đạt chuẩn quốc tế.")
    ...
    private Voucher createVoucher(String code, DiscountType type, String value, int maxUsage, int perUserLimit,
